...
public class ContextTests : DatabaseTest
{
    ...
    [Fact]
    public void DbiPostgresComparisonInsert()
    {
        _db.Database.EnsureDeleted();
        _db.Database.EnsureCreated();
        ...
        _db.SaveChanges();
        watch.Stop();
        _output.WriteLine($"Inserting 100.000 playlists took {watch.ElapsedMilliseconds} ms");
    }

    [Fact]
    public void DbiPostgresComparisonSelect()
    {
        var query = from playlist in _db.Playlists
                    select new
                    {
        ...
        watch.Stop();
        _output.WriteLine($"Slecting without filter from 1000 playlists took {watch.ElapsedMilliseconds} ms");

        foreach (var playlist in results)
        {
            _output.WriteLine($"Playlist Title: {playlist.PlaylistTitle}, Playlist ID: {playlist.PlaylistId}");
            foreach (var song in playlist.Songs)
            {
                _output.WriteLine($"\tSong Title: {song.SongTitle}, Song ID: {song.SongId}");
            }
        }
    ...
    [Fact]
    public void DbiPostgresComparisonSelectWithFilter()
    {
        var query = from playlist in _db.Playlists
                    where playlist.IsPublic == true
                    select new
                    {
                        PlaylistTitle = playlist.Title,
                        PlaylistId = playlist.Id,
        ...
        var results = query.ToList();
        watch.Stop();
        _output.WriteLine($"Selecting from 1000 playlists with filter took {watch.ElapsedMilliseconds} ms");
    ...
    [Fact]
    public void DbiPostgresComparisonSelectWithFilterAndSort()
    {
        var query = from playlist in _db.Playlists
                    where playlist.IsPublic == true
                    select new
                    {
                        ...
                        Songs = playlist.Songs.Select(song => new
                        {
                            SongTitle = song.Titel,
                            SongId = song.Id
                        }).OrderBy(song => song.SongTitle).ToList()
                    };

        var watch = Stopwatch.StartNew();
        var results = query.ToList();
        watch.Stop();
        _output.WriteLine($"Selecting from 1000 playlists with filter and sort took {watch.ElapsedMilliseconds} ms");
    ...
    [Fact]
    public void DbiPostgresComparisonDelete()
    {
        var allPlaylists = _db.Playlists.ToList(); 
        _db.Playlists.RemoveRange(allPlaylists);

        var watch = Stopwatch.StartNew();
        _db.SaveChanges();
        watch.Stop();
        _output.WriteLine($"Deleting of 1k took {watch.ElapsedMilliseconds} ms");
    }

    [Fact]
    public void DbiPostgresComparisonUpdate()
    {
        var allPlaylists = _db.Playlists.ToList();

        foreach (var playlist in allPlaylists)
        {
            // Use Faker to generate a random title
            playlist.Title = new Faker().Lorem.Word(); 
        }

        var watch = Stopwatch.StartNew();
        _db.SaveChanges();
        watch.Stop();
        _output.WriteLine($"Updating the playlist tiles of 1k took {watch.ElapsedMilliseconds} ms");
    }
